/**
 * GovMap-councils write — ADDITIVE append of the 9 GovMap-geocoded mikveh records
 * (Tel Aviv 3 + Bat Yam 6) into the local generated app dataset. Same safety
 * contract as apply-ashkelon.ts: BACKUP first, APPEND only (NEVER rebuild),
 * recompute cities additively, THROW + restore on any reconciliation failure.
 * No publish, backlog untouched.
 *
 * Run:  node importers/mikvahs/apply-govmap-councils.ts
 */
...
const PLACES_BAK = join(GEN, 'places.osm.pre-govmap-councils.backup.json');
const CITIES_BAK = join(GEN, 'cities.osm.pre-govmap-councils.backup.json');
const REPORT = join(OUT, 'govmap-councils-apply-result.json');

const read = <T>(p: string): T => JSON.parse(readFileSync(p, 'utf8')) as T;
const write = (p: string, d: unknown): void => writeFileSync(p, JSON.stringify(d, null, 2), 'utf8');

const EXPECTED = 9;
const EXPECTED_MIKVEH_BEFORE = 606;

function run(): void {
  const before = read<Place[]>(PLACES);
  const incoming = read<Place[]>(join(OUT, 'govmap-councils-write-ready-preview.json'));
  ...
  try {
    ...
    const result = {
      status: 'SUCCESS',
      scope: '9 GovMap-geocoded official mikveh records (Tel Aviv 3 + Bat Yam 6), all locationPrecision=address. additive append. rebuildAppDataset NOT run. Backlog untouched. No publish.',
      ...
      added: incoming.map((p) => ({ id: p.id, name: p.name, city: p.cityId, geocode: (p.extra as any)?.geocodeLabel })),
      license: 'council-public; coordinates via GovMap ADDR_V1 (official Israeli address DB), ITM→WGS84',
    ...
    write(REPORT, result);
    console.log('=== GovMap-councils WRITE — SUCCESS (additive) ===');
