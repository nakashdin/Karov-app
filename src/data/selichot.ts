...
export function elulDetails(state: ElulState): DetailBlock[] {
  ...
  return [
    ...
    {
      heading: 'לְדָוִד ה׳ אוֹרִי וְיִשְׁעִי',
      text: 'נוהגים לומר את פרק כ״ז בתהילים פעמיים בכל יום — בבוקר ובערב — מראש חודש אלול ועד יום הכיפורים, לאורך ארבעים ימי הרצון. יש הממשיכים באמירתו עד שמיני עצרת (מטה אפרים תקפ״א, ו).',
    },
    {
      text: 'חז״ל דרשו את פתיחת המזמור על ימי הדין עצמם: ״אוֹרִי — בראש השנה, וְיִשְׁעִי — ביום הכיפורים״ (מדרש תהילים כ״ז, ג). ומכאן מקומו דווקא בימים אלו.',
    },
    {
      text: 'בפרק מופיע שם הוי״ה שלוש עשרה פעמים, כנגד שלוש עשרה מידות הרחמים. הזוהר פותח בדימוי הזה על הפסוק ״כְּשׁוֹשַׁנָּה בֵּין הַחוֹחִים״ (שיר השירים ב, ב): כשם שבשושנה שלושה עשר עלים, כך כנסת ישראל מוקפת שלוש עשרה מידות של רחמים מכל צדדיה (זוהר, הקדמה, דף א ע״א).',
    },
    {
      text: 'ואף ארבעים הימים אינם מספר בעלמא — הם אותם ארבעים יום שבהם עלה משה רבנו להר בשנייה, וירד ביום הכיפורים עם הבשורה שנמחל לעם.',
    },
  ];
}
